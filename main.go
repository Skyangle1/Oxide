...
func messageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if strings.HasPrefix(lowerContent, "lyre") || strings.HasPrefix(lowerContent, "queen") {
		...
		if !AllowedUsers[m.Author.ID] {
			...
			s.ChannelMessageSend(m.ChannelID, "❌ **PERINGATAN KERAS!** ❌\n\nLu bukan admin bre, jangan ngasal pake perintah bot gwejh. Bot ini cuma buat 2 orang aja anjg. Cabut sana kalo gak diizinin, jangan ganggu kami main musik disini. Kalo nekat mampus lu ntar gua kick dari server.")
			return
		}
...
func playNextTrack(s *discordgo.Session, guildID string, channelID string) {
	...
	// Reuse existing voice connection if it's still active, otherwise join
	if existingVc != nil && existingVc.Ready && existingVc.ChannelID == channelID {
		...
		// Join voice channel since no active connection exists
		// Enhanced retry mechanism for handling 4016 errors during voice connection
		maxRetries := 5  // Increased retries
		retryCount := 0

		for retryCount < maxRetries {
			// Only disconnect if we have an existing connection that's not ready or on wrong channel
